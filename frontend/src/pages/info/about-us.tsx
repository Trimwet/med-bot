  ...
  useEffect(() => {
    getPublicStats()
      .then((data: PublicStats) => {
        const hospCount = data.hospitals || 120
        const patCount = data.patientsTriaged || 50000
        const formattedPatients = patCount >= 1000 ? `${Math.round(patCount / 1000)}K+` : `${patCount}+`
        setStats([
          { value: `${hospCount}+`, label: 'Hospitals' },
          { value: formattedPatients, label: 'Patients triaged' },
          { value: '40%', label: 'Wait time reduction' },
          { value: `${data.uptime || 99.9}%`, label: 'Uptime' },
        ])
      })
      .catch(() => {})
  }, [])
    ...
    >
      ...
      <div className="bg-white rounded-2xl p-8 md:p-10 shadow-sm border border-gray-100">
        ...
        <div className="space-y-4 text-gray-600 leading-relaxed text-base">
          ...
          <p>
            Today, MedBot is used across {stats[0]?.value || '120+'} hospitals, helping clinicians make faster, more informed
