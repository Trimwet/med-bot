  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500">May 25 - May 31, 2026</p>
          </div>
          ...
          {weeklyStats.map((stat) => (
            <div key={stat.label} className="text-center">
              <div className="flex items-center justify-center gap-1 mb-1">
                ...
                <span className="text-[10px] text-gray-400 dark:text-gray-500 uppercase">{stat.label}</span>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 pt-4 border-t border-gray-100 dark:border-gray-700">
          ...
          <div>
            <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Top Symptoms Reported</h4>
            <div className="space-y-2">
              {weeklySymptoms.map((s) => (
                <div key={s.name}>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-600 dark:text-gray-400">{s.name}</span>
                    <span className="text-gray-500 dark:text-gray-400">{s.count} ({s.percent}%)</span>
                  </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="font-semibold text-gray-900 dark:text-gray-100">Monthly Report</h3>
            <p className="text-xs text-gray-400 dark:text-gray-500">May 1 - May 31, 2026</p>
          </div>
          <button className="flex items-center gap-2 px-4 py-2 bg-[#073B4C] text-white rounded-lg text-sm font-semibold hover:bg-[#0A202A] transition-colors">
            <Download className="w-4 h-4" />
            <span className="hidden sm:inline">Download Report</span>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
          {monthlyStats.map((stat) => (
            <div key={stat.label} className="text-center">
              <p className="text-[10px] text-gray-400 dark:text-gray-500 uppercase mb-1">{stat.label}</p>
              <p className="text-xl font-bold text-gray-900 dark:text-gray-100">{stat.value}</p>
              <span className="text-[10px] text-green-600 font-semibold">{stat.change}</span>
            </div>
          ))}
        ...
        <div className="pt-4 border-t border-gray-100 dark:border-gray-700">
          <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Insights & Trends</h4>
          <div className="space-y-2">
            <div className="bg-blue-50 dark:bg-blue-900/20 border-l-4 border-blue-400 p-3 rounded-r-lg">
              <p className="text-sm font-medium text-gray-700 dark:text-gray-300">Highest growth in 6 months</p>
              <p className="text-xs text-gray-500 dark:text-gray-400">The 18% increase in assessments is the highest recorded growth since November 2025.</p>
            </div>
            <div className="bg-green-50 dark:bg-green-900/20 border-l-4 border-green-400 p-3 rounded-r-lg">
              <p className="text-sm font-medium text-gray-700 dark:text-gray-300">Efficiency Peak</p>
              <p className="text-xs text-gray-500 dark:text-gray-400">Referral processing time dropped by 24% this month due to new AI sorting algorithms.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="font-semibold text-gray-900 dark:text-gray-100">Yearly Report</h3>
            <p className="text-xs text-gray-400 dark:text-gray-500">Apr 1 - Jun 30, 2026</p>
          </div>
          <button className="flex items-center gap-2 px-4 py-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg text-sm font-semibold hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 pt-4 border-t border-gray-100 dark:border-gray-700">
          <div>
            <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Quarterly Summary</h4>
            <ul className="text-sm text-gray-600 dark:text-gray-400 space-y-1 list-disc list-inside">
          ...
          <div>
            <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Quarterly Top Symptoms</h4>
            <div className="space-y-2">
              {yearlySymptoms.map((s) => (
                <div key={s.name}>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-600 dark:text-gray-400">{s.name}</span>
                    <span className="text-gray-500 dark:text-gray-400">{s.count.toLocaleString()} ({s.percent}%)</span>
                  </div>
                  <div className="w-full h-1.5 bg-gray-100 dark:bg-gray-700 rounded-full mt-1">
