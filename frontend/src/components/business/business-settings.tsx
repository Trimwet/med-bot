...
const settingsItems = [
  ...
  },
  {
    ...
    action: 'Logout',
    actionStyle: 'text-red-500 border-red-200 hover:bg-red-50',
  },
]
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="space-y-3">
        {settingsItems.map((item) => (
          <div
            key={item.title}
            className="bg-white rounded-xl border border-gray-200 p-5 flex items-center justify-between hover:shadow-sm transition-shadow cursor-pointer"
          >
            ...
            <div className="flex items-center gap-2">
              <button
                className={`px-4 py-2 border rounded-lg text-sm font-semibold transition-colors ${
                  item.actionStyle || 'border-gray-200 text-gray-700 hover:bg-gray-50'
                }`}
              >
                {item.action}
              </button>
